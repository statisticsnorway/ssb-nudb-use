...
def identify_cols_not_in_keep_drop_in_paths(paths: list[Path], cols_keep: list[str], cols_drop: list[str], raise_error_found: bool = False):
    ...
    for path in paths:
        ...
        extra_cols = (
            extra_cols | set([col for col in columns_in_data 
                              if col not in cols_keep and 
                                 col not in cols_drop])
        )
    if extra_cols and raise_error_found:
        raise KeyError(f"These columns are missing from keep / drop, please define them in your config or keep+drop lists: {extra_cols}")
        
    return extra_cols

def check_column_presence(df: pd.DataFrame,
                          dataset_name: str | None = None,
                          check_for: None | list[str] = None,
                          raise_errors: bool = True) -> list[Exception]:
    ...
    """
    with LoggerStack(f"Checking for column presence in dataframe for dataset: {dataset_name}"):
        ...
        if not errors:
            logger.info("All columns exist in the datasets config, or in the provided check_for list.")
        return errors
...
def _get_klass_codelist(df: pd.DataFrame,
                        col_codelist: dict[str, list[str] | dict[str, str]] | None = None,
                        full_timeline: bool = False) ->  dict[str, list[str] | dict[str, str]]:
    ...
    if not (isinstance(col_codelist, dict) and 
           all(isinstance(k, str) for k in col_codelist) and
           all(isinstance(v, list | str) for v in col_codelist.values())
           ):
        ...
        for col in df.columns:
            ...
            if col in variables and variables[col]["klass_codelist"]:
                ...
                earliest_version_date, latest_version_date = _find_earliest_latest_klass_version_date(variables[col]["klass_codelist"])

                if full_timeline and earliest_version_date != latest_version_date:
                    col_codelist |= {
                        col: list(
                            klass.KlassClassification(variables[col]["klass_codelist"])
                            .get_codes(from_date=earliest_version_date, to_date=latest_version_date)
                            .to_dict().keys()
                        )
                    }
                else:
                    col_codelist |= {
                        col: list(
                            klass.KlassClassification(variables[col]["klass_codelist"])
                            .get_codes().to_dict().keys()
                        )
                    }

            elif col in variables and variables[col]["klass_variant"]: 
                col_codelist |= {
                    col: klass.KlassVariant(variables[col]["klass_variant"]).data["code"].to_list()
                }

    return col_codelist


def check_cols_against_klass_codelists(df: pd.DataFrame, col_codelist: dict[str, list[str] | dict[str, str]] | None = None) -> None:
    ...
    for col, codes in col_codelist.items():
        ...
        if isinstance(codes, list):
            ...
            raise TypeError(f"Codes does not seem to have correct datatype. {type(codes)}")

        outside_df = df[(~df[col].isin(codelist)) & (df[col].notna())]

        if outside_df.shape[0]:
            logger.warning(f"Codes in {col} outside codelist: {outside_df[col].unique()} - first 40 valid values: {list(codelist)[:40]}...")
            outside_df_earliest = df[(~df[col].isin(earliest_codelist)) & (df[col].notna())]

            if not outside_df_earliest.shape[0]:
                logger.info("BUT Codes are not outside the list if we check all the way back the first version.")
            else:
                logger.warning(f"Some codes are even outside the codelist in {col} if we check all the way back the first version: {outside_df_earliest[col].unique()} - first 40 valid values: {list(earliest_codelist)[:40]}...")

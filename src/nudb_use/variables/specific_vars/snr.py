...
def generate_uuid_for_snr_with_fnr_col(df: pd.DataFrame, snr_col: str = "snr", fnr_col: str = "fnr") -> pd.DataFrame:
    ...
    with LoggerStack(f"Generating UUID4 into {snr_col} based on unique values in {fnr_col}"):
        unique_fnr_missing_snr = df[df[snr_col].isna()][fnr_col].dropna().unique()  # The dropna is important, so we dont get a fnr = NA join-key.
        fnr_uuid_katalog = pd.DataFrame({fnr_col: unique_fnr_missing_snr,
                                    "uuid": [str(uuid.uuid4()) for _ in unique_fnr_missing_snr]
                                    })
        amount_na_pre_first_fill = df[snr_col].isna().sum()
        df[snr_col] = df[snr_col].fillna(df.merge(fnr_uuid_katalog, on=fnr_col, how="left", validate="m:1"))
        ...
        logger.info(f"Filled {percent_diff}% of `{snr_col}` with UUIDs based on unique, non-missing values in `{fnr_col}`")
        
        if amount_na_post_first_fill:
            logger.warning(f"""Still empty cells in `{snr_col}` after filling from unique values in `{fnr_col}`, `{fnr_col}` might contain NA-values? 
...
def generate_uuid_for_snr_with_fnr_catalog(df: pd.DataFrame,
                                           fnr_catalog_path: str | Path,
                                           snr_col: str = "snr", 
                                           fnr_col: str = "fnr") -> pd.DataFrame:
    ...
    with LoggerStack(""):
        ...
        catalog_path = Path(fnr_catalog_path)
        if catalog_path.exists():
            catalog = pd.read_parquet(catalog_path)
        else:
            logger.info(f"Fnr-uuid catalog does not exist, so we are starting with an empty one, and writing to: {fnr_catalog_path}")

        # Apply the previously generated uuids into the snr_col
        df[snr_col] = df[snr_col].fillna(df.merge(catalog, on=fnr_col, how="left")[snr_col])
        ...
        filled_fnr_snr = df[snr_missing_pre_generate_mask & (df[snr_col].notna())][[fnr_col, snr_col]].drop_duplicates()
        # Write updated catalog back to drive with added generated values
        catalog = pd.concat([catalog, filled_fnr_snr]).drop_duplicates().reset_index(drop=True)

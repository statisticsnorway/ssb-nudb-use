...
def values_not_in_column(col: pd.Series, values: list[Any] | Any, raise_error: bool = False) -> None | ValueError:
    ...
    if len(values_in_col):
        err = ValueError(f"Values in col that shouldnt be there: {values_in_col.unique()}, amount of rows: {len(values_in_col)}")
        if raise_error:
            raise err
        return err
    return None

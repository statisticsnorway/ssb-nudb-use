...
def check_nus2000(df: pd.DataFrame, **kwargs) -> list[NudbQualityError]:
    ...
    with LoggerStack("Validating for specific variable: nus2000"):
        ...
        add_err2list(errors, subcheck_nus2000_uh_institusjon_id_against_nus(
...
def subcheck_nus2000_valid_range(
    ...
    **kwargs: Any,
    ...
    if range_valid_nus is None and "dataset_name" in kwargs:
        logger.info(f"Trying to fetch nus2000 valid range from config for dataset {kwargs['dataset_name']}.")
    ...
    mask: pd.Series = ((vals < range_level5_min) | (vals > range_level5_max)).fillna(False)
...
def subcheck_nus2000_uh_institusjon_id_against_nus(uh_institusjon_id_col: pd.Series, nus_col: pd.Series, utd_skoleaar_start_col: pd.Series) -> NudbQualityError | None:
    ...
    if args_have_None(hskode = uh_institusjon_id_col, nus2000 = nus_col, utd_skoleaar_start = utd_skoleaar_start_col):
        return None
    ...
    if len(nuscodes_nonvalid_hskode):
        err = NudbQualityError(f"Found invalid {uh_institusjon_id_col.name} on nuscodes (when first digit of nus2000 is 6, 7 or 8, {uh_institusjon_id_col.name} should not be empty or 999). Take a look at these nuscodes: {nuscodes_nonvalid_hskode}")

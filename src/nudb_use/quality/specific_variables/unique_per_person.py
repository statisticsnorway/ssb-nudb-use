...
def check_unique_per_person(df: pd.DataFrame, **kwargs) -> list[NudbQualityError]:
    ...
    unique_cols_in_df = [col for col in df.columns if col.lower() in UNIQUE_PER_PERSON_COLS]
    if len(unique_cols_in_df):
        with LoggerStack(f"Checking columns that should be unique per person: {UNIQUE_PER_PERSON_COLS}"):
            for unique_col in unique_cols_in_df:
                add_err2list(errors, subcheck_unique_per_person(fnr, pers_id, df[unique_col], unique_col))
            
    return errors


def subcheck_unique_per_person(fnr: pd.Series, pers_id: pd.Series, unique_col: pd.Series, unique_col_name: str) -> NudbQualityError | None:

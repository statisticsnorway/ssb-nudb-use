...
KOMMUNE_VARS = [var_name for var_name, var_details in settings.variables.items() if var_details.get("klass_codelist") == 131]
...
def check_kommune(df: pd.DataFrame, **kwargs) -> list[NudbQualityError]:
    ...
    if not kommune_vars_in_df:
        return errors
    with LoggerStack(f"Validating specific kommune-variables that link to klass-codelist 131: {kommune_vars_in_df}"):
        for kom_col_name in kommune_vars_in_df:
            kom_col = get_column(df, col = kom_col_name)
            add_err2list(errors, subcheck_single_kommune_oslo_svalbard_utland(kom_col, kom_col_name))
            add_err2list(errors, subcheck_only_single_sentinel_value_9999_allowed(kom_col, kom_col_name))
        return errors
        
def subcheck_single_kommune_oslo_svalbard_utland(kommune_col: pd.Series, col_name: str) -> NudbQualityError | None:
    ...
    non_match = unique_vals_in_legal[unique_vals_in_legal != unique_vals_in_legal.str[:2].map(legal_vals)]
...
def subcheck_only_single_sentinel_value_9999_allowed(kommune_col: pd.Series, col_name: str) -> NudbQualityError | None:
    if args_have_None(kommune_col=kommune_col, col_name=col_name):
        return None
    unique_vals = pd.Series(kommune_col.dropna().unique())
    mask_weird = (unique_vals.str.startswith("00") | unique_vals.str.startswith("9")) & (unique_vals != "9999")

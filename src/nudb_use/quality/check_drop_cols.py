...
def check_drop_cols_for_valid_cols(drop_cols: list[str], ignores: list[str] | str | None = None, raise_errors: bool = False):
    ...
    with LoggerStack("Looking for columns in your drop that you might want to keep."):
        ...
        renamed_list = [x for y in (var_meta_valid
        .query("~renamed_from.isna()")
        ["renamed_from"].to_list()) for x in y ]
        ...
        if ignores is not None:
                if isinstance(ignores, str):
                    ...
                    ignore_list = ignores
                overlap = [c for c in overlap if c not in [col.lower() for col in ignore_list]] 

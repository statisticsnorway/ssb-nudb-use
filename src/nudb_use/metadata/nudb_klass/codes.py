...
def get_klass_codes(klassid: int, 
    ...
    if data_time_start is None and data_time_end is None:
        ...
        codes = klass.KlassClassification(klassid).get_codes(from_date=data_time_start, to_date=data_time_end)
    elif data_time_start is None:
        raise ValueError("If you specify the end, you MUST also specify the start date (or just the start).")  
    else:
        raise ValueError("What the hell are you doing with the date parameters?")    
    return list(codes.to_dict().keys())


def check_klass_codes(df: pd.DataFrame,
                      data_time_start: str | None = None,
                      data_time_end: str | None = None,
    ...
    with LoggerStack("Checking if column-content matches codelists in KLASS"):
        ...
        for col in df.columns:
            if col not in metadata.index:
                ...
                logger.debug(f"Not checking `{col}`, its not supposed to have a codelist, the codelist-int is 0.")
                ...
                to_date = None if from_date is None else datetime.datetime.now().strftime("%Y-%m-%d")
                ...
                outside_df = df[(~df[col].isin(codes)) & 
                             (df[col].notna())]
                
                if len(outside_df):
                    errors += [NudbQualityError(f"Codes in {col} outside codelist: {outside_df[col].unique()}")]
                else:
